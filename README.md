...
- 基础:goto(坐标)、goto_block(找最近方块)、mine(采集,可限数量,带收获校验)、follow(跟随)、wait、say、stop、remember/forget(地点记忆)
- **合成与熔炼**:craft(配方书驱动,自动用工作台,"确保背包里有 N 个"语义)、smelt(熔炉全流程:走过去→装料装燃料→等待→取出,带燃料耗尽检测)
...
被打自动暂停任务反击、威胁解除自动继续、死亡自动终止任务并把死亡点记为记忆地点(复活后说"去死亡点"即可回去捡装备)。
